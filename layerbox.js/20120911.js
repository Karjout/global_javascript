...
YAHOO.ebauer.layerbox = function () { // escape if less than ideal environment
    ...
    // configObj['added'] = {name:"added", width:593, height:445,
    // cmPageName:'/js/eb/item_added_to_cart.js',functionName:function()
    // {prepareItemForm(YAHOO.ebauer.productUtils.isEOBProduct);}};
    configObj['added'] = {
        name: "added",
        width: 593,
        height: 445,
        cmPageName: '/js/eb/item_added_to_cart.js (Interstitial Page)',
        functionName: function () {
            prepareItemForm(YAHOO.ebauer.productUtils.isEOBProduct);
        }
    };
    // TSK01145 - Remvoe KIOSK: from Coremetrics tracking
    configObj['addedFromKiosk'] = {
        name: "addedFromKiosk",
        width: 593,
        height: 445,
        cmPageName: '/js/eb/item_added_to_cart.js (Interstitial Page)',
    ...
    configObj['addedToWishList'] = {
        name: "addedToWishList",
        width: 593,
        height: 560,
        cmPageName: '/js/eb/item_added_to_wishlist.js',
        functionName: function () {
            prepareWishListItemForm(YAHOO.ebauer.productUtils.isEOBProduct);
        }
    };
    configObj['buyanother'] = {
        name: "product",
        width: 593,
        height: 445,
        cmPageName: '/js/eb/productdata.js',
        path: "dossier.jsp"
    };
    configObj['content'] = {
        name: "content",
        ...
        height: 430,
        cmPageName: '',
        path: "get_product_info.jsp"
    };
    configObj['eobproduct'] = {
        name: "product",
        width: 970,
        height: 540,
        cmPageName: '/js/eb/productdata.js',
        path: "eob_dossier.jsp"
    };
    configObj['giftbox'] = {
        name: "giftbox",
        width: 475,
        height: 520,
        cmPageName: '/js/eb/giftbox.js',
        functionName: function () {
            createGiftBox(YAHOO.ebauer.productUtils.passedItemUUID,
					YAHOO.ebauer.productUtils.siteCss);
        }
    };
    configObj['monogram'] = {
        name: "monogram",
        width: 593,
        height: 550,
        cmPageName: '/js/global/monogram.js',
        functionName: function () {
            loadMonogramDetailsForItem(
					YAHOO.ebauer.productUtils.passedItemUUID,
					YAHOO.ebauer.productUtils.siteCss);
            //getCloseX();
        }
    };
    ...
    configObj['removed'] = {
        ...
        width: 593,
        height: 450,
        cmPageName: '/js/eb/removed_item_from_cart.js',
        functionName: function (bag) {
            updatedShoppingBag(bag);
        }
    };
    configObj['soldout'] = {
        name: "soldout",
        ...
        cmPageName: '/js/eb/productdata.js',
        functionName: function () {
            YAHOO.ebauer.checkout.createSoldOutLayer();
        }
    };
    configObj['login'] = {
        name: "login",
        width: 475,
        height: 520,
        cmPageName: '/js/eb/login.js',
        functionName: function () {
            YAHOO.ebauer.loginUtils.createLoginLayer();
        }
    };

    var modalAnim;
    var currentState = ""; // holds the current configObj, used to calculate
    // top and left changes for transitions
    var newName = "";
    var masked = false;
    var newLeft = 0;
    var newTop = 0;
    ...
    baseStartWidth = 100;
    baseStartHeight = 100;
    var iFrameLayer = null; // holds the automatic iframe element built by the
    // modal while in IE6

    var setIFrame = function () {
        // if the iFrame shim is in use behind layerbox (defualt behavior for
        // IE6)
        if (YAHOO.ebauer.layerbox.modal.iframe) {
            ...
            YAHOO.ebauer.transitions.onTransitionComplete
					.subscribe(correctIFrame);
        }
    };

    var correctIFrame = function () {
        // makes sure the invisible iframe built by the modal for ie6 is in sync
    ...
    var addLayerboxMaskListener = function () {
        ...
        // This prevent the layer from being closed prematurely, causing some JS
        // errors
        if (!masked) {
            masked = true;
    ...
    var getCloseX = function () {
    	$('#modal_monogram').append('<a id="close" class="closeX" href="javascript:closeProductInfoLayer();"></a>');
    }
    ...
    var editModalListeners = function (newName) {
        ...
        if (currentState != "") {
            ...
            if (currentName == "giftbox") { // submitGiftBox,cancelGiftBox,continueWithout
                $E.removeListener('cancelGiftBox', 'click',
						YAHOO.ebauer.layerbox.cancelClickCallback);
                $E.removeListener('continueWithout', 'click',
						YAHOO.ebauer.layerbox.continueWithoutGiftBox);
            }
            if (currentName == "monogram") { // submitGiftBox,cancelGiftBox,continueWithout
                $E.removeListener('cancelMonogram', 'click',
						YAHOO.ebauer.layerbox.cancelClickCallback);
                $E.removeListener('continueWithout', 'click',
						YAHOO.ebauer.layerbox.continueWithoutMonogram);
            }
            if (currentName == "product") { // submitGiftBox,cancelGiftBox,continueWithout
                $E.removeListener('goBack', 'click',
            ...
            if (currentName == "removed") { // buyAnother,addeditGiftbox,goBack
                $E.removeListener('checkoutButton', 'click', showNextPage);
                $E.removeListener('goBackFromRemoved', 'click',
						YAHOO.ebauer.layerbox.hideLayer);
            }
        ...
        if (newName != undefined || newName != '') {
            addLayerboxMaskListener(); // If you come into layer after the
            // login layer, then you may want to add
            // this listener.
        }
        // BUG01603: Reload parent page for all close buttons except product
        // page
        if (YAHOO.ebauer.productUtils.productPageMode) {
            $E.addListener('closeButtonId', 'click',
					YAHOO.ebauer.layerbox.hideLayer);
        } else {
        ...
        if (newName == "giftbox") { // submitGiftBox,cancelGiftBox,continueWithout
            if (YAHOO.ebauer.productUtils.productPageMode)
                $E.addListener('cancelGiftBox', 'click',
						YAHOO.ebauer.layerbox.hideLayer);
            else
                $E.addListener('cancelGiftBox', 'click',
						YAHOO.ebauer.layerbox.cancelClickCallback,
						backToFormat, false);
            $E.addListener('continueWithout', 'click',
					YAHOO.ebauer.layerbox.continueWithoutGiftBox);
        }

        if (newName == "monogram") { // submitGiftBox,cancelGiftBox,continueWithout
            if (YAHOO.ebauer.productUtils.productPageMode)
                $E.addListener('cancelMonogram', 'click',
						YAHOO.ebauer.layerbox.hideLayer);
            else
                $E.addListener('cancelMonogram', 'click',
						YAHOO.ebauer.layerbox.cancelClickCallback,
        ...
        if (newName == "login") { // submitGiftBox,cancelGiftBox,continueWithout
            if (YAHOO.ebauer.loginUtils.getProductLayerLogin() == true) {
                $E.addListener('cancelLogin', 'click',
						YAHOO.ebauer.layerbox.cancelClickCallback,
						backToFormat, false);
        ...
        if (newName == "added" || newName == "addedFromKiosk") { // buyAnother,addeditGiftbox,goBack
            ...
            if (YAHOO.ebauer.bagUtils) {
                ...
                $E.addListener('goBackFromAdded', 'click',
						YAHOO.ebauer.layerbox.showCategoryFromProductPage);
        ...
        if (newName == "removed") { // buyAnother,addeditGiftbox,goBack
            $E.addListener('checkoutButton', 'click', showNextPage);
            $E.addListener('goBackFromRemoved', 'click',
					YAHOO.ebauer.layerbox.hideLayer);
        }
    };
    ...
    var findCenter = function (desiredWidth, desiredHeight) {
        ...
        if (viewPortWidth < 810)
            newLeft = newLeft + 20; // offsets the layer when it's bigger that
        // the viewport so that all buttons are
        // visible
    };
    ...
    var transitionModal = function (e, formatObj, p_obj) {
        ...
        if (newName == currentState.name) {
            if (newName == "product") {
                // transition from product layer to another product layer delay
                // the load of showProductData until most of dossier has loaded,
                // dash is a div near the bottom of dossier
                YAHOO.util.Event.onContentReady("dash",
						YAHOO.ebauer.layerbox.showProductData, p_obj, true);
            ...
            if (newName == "product") {
                // delay the load of showProductData until most of dossier has
                // loaded, dash is a div near the bottom of dossier
                YAHOO.util.Event.onContentReady("dash",
						YAHOO.ebauer.layerbox.showProductData, p_obj, true);
            }
            if (newName == "monogram")
                YAHOO.util.Event.onContentReady("monogramPreview",
						YAHOO.ebauer.layerbox.showMonogramData, p_obj, true);
            editModalListeners(newName);
            if (currentState == "") {
                p_obj.modal.show();
            }
    ...
    var setCoreMetricsProductData = function () {
        ...
        if (getValueFromElement('eob') == 'true') {
            var pid = getValueFromElement('deptIndex')
					+ getValueFromElement('dept') + ' '
					+ getValueFromElement('firstEffort') + ' '
					+ getValueFromElement('item');
            piValue = 'T50';
        } else {
    ...
    return {
        ...
        init: function () {
        	...
        	YAHOO.ebauer.layerbox.modal = new YAHOO.widget.LayerBox('layerbox',
					{
					    state: 0,
					    effect: {
        ...
        launch: function (e, el) {
            ...
            if (elTarget) {
                ...
                if (ensembleId != "") {
                	//var pdpURL = rewirteUrl('/catalog/product.jsp?standalone=N&ensembleId='+tempPid+'&cs=1&ts='+timestamp);
                	//fb.start(pdpURL, 'type:ajax height:600 width:968 autoFitHTML:true scrolling:no padding:0 panelPadding:20 outerClose:true controlsPos:tr afterItemStart:YAHOO.ebauer.productUtils.changeWeatheredgeCSS();');
                	
                	var pplURL = rewirteUrl('/catalog/product.jsp?standalone=N&ensembleId='+tempPid+'&cs=1&ts='+timestamp);
              		var pplType = 'ajax';
        ...
        overlay: function (formatStr, passedIdObj) {
            // used to display and overlay layer on top of the layerbox, as in a
            // size chart on the product layer
            ...
            if (YAHOO.ebauer.transitions.checkOverlayStatus()) {
                // if an overlay is already open and we switch to another one
                // concat formatStr and passedIdObj into one str that can get
                // passed along to the reopen once the close anim is complete
        ...
        clearEnsembleVars: function () {
            ...
            if (!YAHOO.ebauer.productUtils.productPageMode) {
                // don't reset these if the call is coming from the stand alone
                // product page
                colorValue = null;
                colorIndex = 0;
                ensembleId = "";
                ...
                YAHOO.ebauer.productUtils.isGiftBoxSelected = false; // reset
                // the
                // switch
                YAHOO.ebauer.productUtils.isMonogramSelected = false; // reset
                // the
                // switch
                YAHOO.ebauer.productUtils.editProductMode = false; // reset the
                // switch
                YAHOO.ebauer.productUtils.editProductModeType = "";
                YAHOO.ebauer.productUtils.passedItemUUID = null;
                YAHOO.ebauer.productUtils.buyAnotherMode = false; // reset the
                // switch
                ...
                YAHOO.ebauer.productUtils.resetFlags(); // Reset the flag for
                // inventory display as
                // well.
        ...
        hideLayer: function (e) {
            if (e && e.srcElement) {
                if (e.srcElement.id == 'goBackFromAdded'
						|| e.srcElement.id == 'goBackFromRemoved'
						|| e.srcElement.id == 'goBack') {
                    // Let us generate the cm tag
                    var targetUrl = document.location.href;
                    var ppl = "{type:'hide'}";
                    var newTargetUrl = targetUrl.replace(targetUrl.substring(
							targetUrl.indexOf("#"), targetUrl.length), "#ppl="
							+ encodeURIComponent(ppl));
        ...
        closeInfoLayer: function () {
            // if this is a layer on a layer (like a size chart on a product
            // layer), just hide the top layer
            if (YAHOO.ebauer.transitions.checkOverlayStatus())
                YAHOO.ebauer.transitions.transitionOverlay();
            // otherwise, if this is the parent layer (like a size chart on
            // multi-sku), run hideLayer
            else
                YAHOO.ebauer.layerbox.hideLayer();
        },
        setCut: function () {
        ...
        checkProductLayerModules: function () {
            ...
            if (YAHOO.ebauer.productUtils.productLayerType == "REG") {
                ...
                // so just skip this flag and wait for the swatch and info
                // modules
                additionalModulesHaveRun = true;
            }

            if (YAHOO.ebauer.productUtils.infoLoaded === true
        ...
        showProductData: function () {
            ...
            if (!showSelectedValues && !showSelectedValuesForBuyAnotherItem) {
                if (getValueFromElement('eob') != 'true'
						&& getValueFromElement('displayClearaceVariants') != 'Y')
                    updateOutFitsModule();
                else if (getValueFromElement('eob') == 'true')
            ...
            if (isClearanceCategory == 'Y') {
                ...
                if (colorIdSelected && sizeIdSelected && colorIdSelected != -1
						&& sizeIdSelected != -1) {
                    selectedClxColorSize = colorIdSelected + '-'
							+ sizeIdSelected;
                }
            ...
            // var firstCutIndex = 1;
            var inseamFlagEl = $("inseamFlag_" + selectedCutIndex); // BUG01175:
            // Inseam
            // Length
            // dropdown
            // box
            ...
            if (inseamFlagEl != null) {
                ...
                inseamFlagEl = $D.getElementsBy(findInseamFlag, 'input',
						'productForm');
            ...
            if (getValueFromElement('eob') == 'true') {
                ...
                if (thisProdId != -1) {
                    ...
                    createDTMAbandonPageTag("5", document
							.getElementById("userId").value, document
							.getElementById("primaryCategoryName").value,
							document.getElementById("defaultImageUrl").value,
							thisProdId);
                    var matte = document.getElementById("matte");
                    var myDivElement = createDOM('div', {
                        'id': 'dtmdiv',
                        'style': 'display:none'
                    });
                    myDivElement.innerHTML = '<iframe name="response_frame" src="'
							+ dtmSrc + '"></iframe>';
                    matte.appendChild(myDivElement);
                }
            } else {
                createDTMAbandonPageTag("5",
						document.getElementById("userId").value, document
								.getElementById("primaryCategoryName").value,
						document.getElementById("defaultImageUrl").value,
						document.getElementById("ensembleId").value);
                var matte = document.getElementById("matte");
                var myDivElement = createDOM('div', {
                    'id': 'dtmdiv',
                    'style': 'display:none'
        ...
        showProductDataBasic: function () {
            ...
            updateShownIn(); // update the shown in display initially, other
            // updates are done through swatch select
            YAHOO.ebauer.ieselect.init();
            YAHOO.ebauer.layerbox.setProductLayerType();
            setCoreMetricsProductData();
        },
        continueWithoutGiftBox: function () {
            if (YAHOO.ebauer.productUtils.editProductMode
					&& YAHOO.ebauer.productUtils.isGiftBoxSelected) {
        ...
        continueWithoutMonogram: function () {
            if (YAHOO.ebauer.productUtils.editProductMode
					&& YAHOO.ebauer.productUtils.isMonogramSelected) {
        ...
        showCategoryFromProductPage: function (e) {
            ...
            if (categoryUrlNode != undefined) {
                ...
                // alert("categoryUrlNode != undefined, caturl = " +
                // categoryUrl);
                // BUG01291 - trimmed off any #string that might be set from a
                // searched product
                var hashCharPos = categoryUrl.indexOf("#");
                if (hashCharPos != undefined && hashCharPos > 0) {
                    categoryUrl = categoryUrl.substring(0, hashCharPos);
                    // alert("trimmed off # strings on categoryUrl = " +
                    // categoryUrl);
                }
            }
            ...
            if (e && e.srcElement) {
                ...
                if (e.srcElement.id == 'goBackFromAdded') {
                    // Let us generate the cm tag
                    // alert('Function: showCategoryFromProductPage: generate
                    // cmLinkClick Tag');
                    var targetUrl = '';
                    if (categoryUrl != '') {
            ...
            if (categoryUrl != '') {
                // alert("######categoryUrl.indexOf(#)=" +
                // categoryUrl.indexOf("#"));
                if (pageSelected != '' && parseInt(pageSelected) > 1) {
                    if (categoryUrl.indexOf("?") < 0) {
                        categoryUrl = categoryUrl + "?page=" + pageSelected;
                    } else {
                        categoryUrl = categoryUrl + "&page=" + pageSelected;
                    }
                }
        ...
        transitionContentLayer: function (sectionId, sectionName, x) {
        	...
        	if (currentState == "") {
        		...
        		sizeChartDetails(sectionId, sectionName);
//                YAHOO.ebauer.layerbox.transition('content', {
//                    sectionId: sectionId,
//                    sectionName: sectionName
//                });
            } else {
                // if the layerbox is already open, then launch the overlay
                YAHOO.ebauer.layerbox.overlay('content', {
                    sectionId: sectionId,
                    sectionName: sectionName
                });
            }
        },
        transitionContentLayerFromStandAlone: function (sectionId, sectionName,
				x) {
            YAHOO.ebauer.layerbox.overlay('content', {
                sectionId: sectionId,
                sectionName: sectionName
            });
            $j('.sizeModule .tabs').tabs(); // target sizeModule and assign
            // jquery tabs to sizeModule
        },
        cancelClickCallback: function (e, formatStr) {
            ...
            if (this.id && this.id == "cancelLogin") {
                ...
                if (YAHOO.ebauer.loginUtils.loginLevel == 1
						&& (YAHOO.ebauer.loginUtils.lastWishListUserItemIdAdded != '-1'
								&& YAHOO.ebauer.loginUtils.lastWishListUserItemIdAdded != undefined && YAHOO.ebauer.loginUtils.lastWishListUserItemIdAdded != '')) {
                    // Yes it is Login Level 0 or 1, and There is a wishlist
                    // item added.
            ...
            if (this.id && this.id == "cancelGiftBox"){
                ...
                YAHOO.ebauer.layerbox.transition(formatStr,
    					YAHOO.ebauer.productUtils.passedIdObjForCancel);
            }
        },
        transition: function (formatStr, passedIdObj, bypassHistory) {
            if (passedIdObj != null && passedIdObj != undefined) {
                YAHOO.ebauer.productUtils.passedIdObjForCancel = passedIdObj; // hold
                // these values for reuse on the cancel buttons
            }
            YAHOO.ebauer.utilities.navigate("ppl",
					'{type:"transition",ensembleId:"' + ensembleId
							+ '",formatStr:"' + formatStr + '",passedIdObj:'
							+ json.stringify(passedIdObj) + ',categoryId:"'
							+ categoryId + '",pathInfo:"' + pathInfo
							+ '",colorId:"' + colorId + '",sizeIdSelected:"'
							+ sizeIdSelected + '",quantitySelected:"'
							+ quantitySelected + '",imageName:"' + imageName
							+ '",imageTypeCode:"' + imageTypeCode
							+ '",catPath:"' + catPath + '",cs:"' + cs + '"}',
					formatStr, passedIdObj);
            // added this here
        ...
        transitionworker: function (formatStr, passedIdObj) {
            ...
            if (modalAnim != null) {
                modalAnim.onComplete.subscribe(
						YAHOO.ebauer.layerbox.transitionCompleteCallback,
						passedStr, true);
            } else {
                YAHOO.ebauer.layerbox
						.transitionComplete(formatStr, passedIdObj);
            }
        },
        ...
        transitionComplete: function (formatStr, passedIdObj) {
            if (currentState.path == "eob_dossier.jsp" && passedIdObj != null) {
                ...
                YAHOO.ebauer.productUtils.queryString = 'dept='
						+ passedIdObj.department + '&effort='
						+ passedIdObj.effortcode + '&item='
						+ passedIdObj.itemNbr + '&source=' + backToValue
						+ '&eobSource=' + backToValue + '&firsteffort='
						+ passedIdObj.firsteffort + '&pricetypecode='
						+ passedIdObj.pricetypecode;
            ...
            if (formatStr != 'content' && formatStr != 'product'
					&& formatStr != 'eobproduct' && formatStr != 'added'
					&& formatStr != 'addedToWishList') {
...
YAHOO.widget.LayerBox.prototype.init = function(el, userConfig) {
	...
	YAHOO.util.Dom.addClass(this.innerElement,
			YAHOO.widget.LayerBox.CSS_LAYERBOX);
	if (userConfig)
...
// OVERRIDE HANDLER FOR THE 'MODAL' PROPERTY WITH SPECIAL ANIMATION-RELATED
// FUNCTIONALITY
YAHOO.widget.LayerBox.prototype.configModal = function(type, args, obj) {
	...
	if (modal) {
		...
		if (!YAHOO.util.Config.alreadySubscribed(this.hideEvent, this.hideMask,
				this))
		...
		if (!YAHOO.util.Config.alreadySubscribed(
			...
			YAHOO.widget.Overlay.windowResizeEvent.subscribe(this.sizeMask,
					this, true);
		if (!YAHOO.util.Config.alreadySubscribed(this.destroyEvent,
...
YAHOO.widget.LayerBox.prototype.showMask = function() {
	if (this.cfg.getProperty('modal') && this.mask) {
		...
		this.sizeMask();
		/*
		 * var o = this.maskOpacity; // disabling opacity transition due to IE
		 * issues.
		 */
		/*
		 * Animation retained (using lineHeight) to maintain timing. Remove
		 * eventually.
		 */
		if (!this.maskAnimIn) {
		...
		// YAHOO.util.Event.addListener(this.mask, 'mousedown',
		// YAHOO.ebauer.layerbox.hideLayer, this, true);
...
/*-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
 ...
 -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~ */
// pass the namespace object as scope,then pass true to use 'this' scope within
// namespace
YAHOO.util.Event.onDOMReady(YAHOO.ebauer.layerbox.init, YAHOO.ebauer.layerbox,
		true);