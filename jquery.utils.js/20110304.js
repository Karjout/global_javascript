...
global.jquery.utils.tooltips = new Object({
	init : function() {
		...
		$j('.toolTip').hover(
			function() {
				var tempHref = $j(this).attr("href");
                // find # and trim everything to the right to fix a ie bug
                var tabIndexId = tempHref.indexOf("#");
                this.contentHref = tempHref.slice(tabIndexId,tempHref.length);
...
global.jquery.utils.products = new Object({
	...
	rowStackManager : function(contextContainer, ignoreList) {
		...
		global.jquery.utils.products.jumpToPreviousProduct();
	},
	...
	jumpToPreviousProduct : function() {
		var hash = window.location.hash.replace("#","").split("|");
		for (var i in hash) {
			if ( hash[i].indexOf("pid:") != -1 ) {
				var pid = hash[i].replace("pid:","");
				var y = $j("#cell_"+pid ).offset().top;
				window.location.hash = "";
				$j(document).scrollTop(y);
				break;
			}
		}
	},
	init : function() {
		...
		if( !global.jquery.utils.common.isCustomerServicePage() && $j("div.cell").length ) {
	...
	init : function() {		
		...
		$j('.ticket_zoom_icon').live("mouseover", function(){
			...
			if(zoomImageFlag == 'false'){// animation not running go ahead and zoom image	
				...
				var position = zoomImage.position();			
				imageTagert = $j(this).closest('li').find('.imageProduct');
				imageSrc = $j(imageTagert).attr('src');	
				imagePosX = $j(imageTagert).position().left-1;	
				imagePosY = $j(imageTagert).position().top-1;
				imageWidth = $j(imageTagert).width();
				imageHeight = $j(imageTagert).height();		
				$j('#console_log').append("<div>imagePosX="+imagePosX+" imagePosY="+imagePosY+"</div>");
		...
		$j('.ticket_zoom_icon_WishList').live("mouseover", function(){
			...
			if(zoomImageFlag == 'false'){// animation not running go ahead and zoom image	
				...
				var zoomImage = $j(this);
				var position = zoomImage.position();			
				imageTagert = $j(this).closest('div.cell').find('.cmPoseImg');
				imageSrc = $j(imageTagert).attr('src');	
				imagePosX = $j(imageTagert).position().left-1;	
				imagePosY = $j(imageTagert).position().top-1;
				imageWidth = $j(imageTagert).width();
...
$j(document).ready(function(){
	...
	// temp fix for flats
	$j('.cell').each(function(idx, item) {
		var Cell = $j(this).is(":contains('rollover')");		
		if ( Cell ){	
		}else{			
