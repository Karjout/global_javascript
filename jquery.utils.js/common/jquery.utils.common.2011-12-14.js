...
global.jquery.utils.common = new Object({
	...
	consoleLog : function(text) {
		// this will leave all of the newest content at the bottom
...
//			}
	...
	applyCheckButtons : function() {
       		$j('body.CHECKOUT #continueCanadianButtonTop').live('click', function(){
			$j('#continueCanadianButtonBottom').click();			
		});
		$j('body.CHECKOUT #payNowTop').live('click', function(){
			$j('#payNow').click();			
		});

		$j('body.CHECKOUT .billingAddressInfo #continueTop').click(function(event) {
			event.stopPropagation();
			if ($j("form[name='addAddressForm']").length > 0 && $j("form[name='addAddressForm']").validate().form()) {
				$j('#continue,#continueBottom').click();
			}
		});

		$j('body.CHECKOUT .shippingAddressInfo #continueTop').click(function(event) {
			event.stopPropagation();
			if ($j("form[name='addAddressForm']").length > 0 && $j("form[name='addAddressForm']").validate().form()) {
				$j('#continue,#continueBottom').click();
			}
		...
		$j('body.CHECKOUT .billingAddressInfo #continueTop').click(function(event) {
			event.stopPropagation();
			if ($j("form[name='editAddressForm']").length > 0 && $j("form[name='editAddressForm']").validate().form()) {
				$j('#continue,#continueBottom').click();
			}
		});

		$j('body.CHECKOUT .shippingAddressInfo #continueTop').click(function() {
			event.stopPropagation();
			if ($j("form[name='editAddressForm']").length > 0 && $j("form[name='editAddressForm']").validate().form()) {
				$j('#continue,#continueBottom').click();
			}
		...
		$j('body.CHECKOUT #continueTop').live('click', function(){
			$j('#continue,#continueBottom').click();
		});
	},
	bloomreachWidget : function(){
		jQuery(".br-sf-widget").mouseover(function(){	
			jQuery(this).addClass('hover');
		});
		 jQuery(".br-sf-widget").mouseout(function(){
			 jQuery(this).removeClass('hover');	
		});
	},
	useFancyboxforAllPopUps : function() {
		// apply basic fancybox module behavior to all floatbox rel anchors
		$j("a[rel='fancybox'], area[rel='fancybox'], a.fancybox, area.fancybox").each(function(){
	...
	init : function() {
		...
		global.jquery.utils.common.applyCheckButtons();
		global.jquery.utils.common.bloomreachWidget();
	}
