...
//		if (subcat.width() == 0) {
//			container.css("width","auto"); // set back to 5 across if subcat nav is hidden in SiteManager
//			rpp = 8; // change back to 8 rows per page if subcat is suppressed
//		}
		container.find("script").remove();
		container
			.before(billboard_19)
			.before(paginator.filter(".top"))
			.before(subcat)
			.after(billboard_34)
			.after(paginator.filter(".bottom"));
	};
	...
	var addProductCount = function() {
		paginator.append("<span class='page-count'> ("+getNumItems()+" items)</span>");
	};
	...
	var rowStack = function() {
		var offset = ($j(window).width()-wrap.width()) / 2; // hack for margin 0 auto
		var containerWidth = container.width();
		cells.each(function(){			
			var el = $j(this);
			console.log(el);
			// this is the base calculation based on current EB styling of content wrapper	
			// get value to equal zero from fist item in row
			//var pos = (620 - offset) + el.position().left;
			var pos = (290 - offset) + el.position().left;
			//jQuery("#console_log").append("<div>offset = "+ offset + "  pos= "+pos+"</div>");
			// this is a hack for the FA site which doesn't size its content wrapper the same as EB
			if ( $j("body").hasClass("FA") )
				pos = el.position().left - 180;
			var wid = el.width();
			//el.append("offset:"+offset+" pos: "+pos+" wid:"+wid+" next: "+el.next().width()+" container: "+containerWidth);
			//jQuery("#console_log").append("<div>pos + wid + el.next().width() = "+ pos + wid + el.next().width() + "  containerWidth= "+containerWidth+"</div>");
			if ( pos + wid + el.next().width() > containerWidth || el.next().length == 0 ) {
				el.prevUntil("div.row").andSelf().wrapAll("<div class='row gettingHere'></div>");
			}
		});
		rows = container.children("div.row");
		//jQuery("#console_log").append("<div>rows = "+rows.length+"</div>");
		var pc = Math.ceil(rows.length/rpp); // page count = number of rows / rows per page
		var bb = rows.eq(0).children(".billboard_1000").length; // adjust threshold by 1 if billboard_1000 exists
		//jQuery("#console_log").append("<div>Page Count = "+pc + "billboard="+bb+"</div>");		
		for (var i=0; i<pc; i++) {
			var start = (i==0) ? 0 : (i * rpp) + bb;
			var end = start + rpp + bb;
		...
		pages.each(function(){
			...
			var a = $j("<a class='page-button' rel='"+num+"' href='#pg:"+(num+1)+"'>"+(num+1)+"</a>");
			a.click(function(){
			...
			if ( !pg.children("div.row").first().children().first().is(breakers) ) {
				// get the previous page's last breaker
				var breaker = pg.prev().children("div.row").children(breakers).last();
				// if there is a breaker in the previous page that is related to the first element, prepend it
				if ( breaker.length ) {
	...
	var setSelectedButton = function(a) {
		...
		if (a) {
			...
			paginator.children("a.page-button[rel="+(page-1)+"]").addClass("selected");
		}
	};
	...
	var adjustPrevNextButtons = function() {
		if (page == 1) {
			paginator.children("a.page-first, a.page-prev").addClass("disabled");
			paginator.children("a.page-prev").attr("href","javascript:void(0)");
		} else {
			paginator.children("a.page-first, a.page-prev").removeClass("disabled");
			paginator.children("a.page-prev").attr("href","#pg:"+(page-1));
		}
		if (page == pages.length) {
			paginator.children("a.page-last, a.page-next").addClass("disabled");
			paginator.children("a.page-next").attr("href","javascript:void(0)");
		} else {
			paginator.children("a.page-last, a.page-next").removeClass("disabled");
	...
	var postProcess = function() {
		...
		$j.cookie("preferences.pagination.showAll",all,{path:"/"});
	};
	...
	return {
		init : function() {
			...
			if ($j.cookie("preferences.pagination.showAll") && $j.cookie("preferences.pagination.showAll") == "true")
				all = true;
