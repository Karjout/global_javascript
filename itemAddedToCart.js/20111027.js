function prepareItemForm(isEOBProduct) {
	...
	form.appendChild(input);
	input = createDOM("input", {type:"hidden",name:"mydescode",value:"-1"});
	form.appendChild(input);
...
function createShoppingBag(shoppingBag, siteCss) {
	...
	var desCode = document.itemform.mydescode.value;
	if (bag.checkoutPage.indexOf('?') >= 0) {
		bag.checkoutPage = bag.checkoutPage + '&ensembleId='+ensembleId+'&desCode='+desCode;
	} else {
		bag.checkoutPage = bag.checkoutPage + '?ensembleId='+ensembleId+'&desCode='+desCode;
	}
	dialogSlat = createDOM('div', {'class':'dialog-slat'});
...
function goBackToCat() {
	if (window.location.search.indexOf('eobSource') >= 0) {
		...
		for ( i in pairs ) {
	        ...
	        if (key == "emsembleId") {
        		var part4 = '?'+key+'='+obj[key];
        	}
        	if (key == "prodsearch") {
        		var part3 = key+'='+obj[key];
        	}
        	if (key == "cm_cg") {
        		var part2 = key+'='+obj[key];
        	}
        	if (key == "cm_sp") {
        		var part1 = key+'='+obj[key];
        	}
        	pathName = part4 + '&' + part3 + '&' + part2 + '&' + part1
        	
        	
	        //console.log('part4 = ' + part4)
	        if (pathName.indexOf('undefined') >= 0) {
	        	window.history.go(-1);
	        }else{
	        	window.location.href = pathName;
	        }
