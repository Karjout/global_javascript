...
var processAddedItemToCart = function(o) {
	...
	if (item != undefined) {
        ...
        if(item.itemType == 'EOB'){// && GetCookie("EOB_Search")=='Yes'
            ...
            cmCreateShopAction5Tag(productId, item.name, item.itemQuantity, itemPrice, departCode, "T50", tempURL);
        } else {
            cmCreateShopAction5Tag(item.ensembleId, item.name, item.itemQuantity, itemPrice, departCode, gPathInfo, tempURL);
        }
        cmDisplayShop5s();
...
var processAddedItemToWishList = function(o) {
	...
	if (item != undefined) {
        ...
        if(item.itemType == 'EOB'){// && GetCookie("EOB_Search")=='Yes'
            productId = item.departmentprefix  +  departCode + ' ' + item.effortcode + ' ' + item.itemNbr;
            //cmCreateShopAction5Tag(productId, item.name, item.itemQuantity, itemPrice, departCode, "T50", tempURL);// We do not want Shop Action Tag for Items added to WishList
        } else {
            //cmCreateShopAction5Tag(item.ensembleId, item.name, item.itemQuantity, itemPrice, departCode, gPathInfo, tempURL); // We do not want Shop Action Tag for Items added to WishList
        }
...
function openMoreInfo(passedIdObj) {
	fb.start( '/ajax/more_info.jsp?ensembleId='+passedIdObj, 'type:ajax autoFitHTML:true scrolling:no padding:0 panelPadding:20 outerClose:true controlsPos:tr ' );
}
...
var processLoadItemForEdit = function(o) {
   ...
   if (x.item.clearanceFlag)
	...
	if (x.item.itemType == "EOB" || x.item.originalItemType == "EOB") {
        ...
        fb.start( '/catalog/eob_product.jsp?standalone=N&uuid='+editedItemUUID+'&editFromPage='+categoryName+'&'+queryString, 'type:ajax height:600 width:996 autoFitHTML:true scrolling:no padding:0 panelPadding:20 outerClose:true controlsPos:tr ' );
	} else {
    	...
    	fb.start( '/catalog/product.jsp?standalone=N&uuid='+editedItemUUID+'&editFromPage='+categoryName+'&ensembleId='+ensembleId+'&'+queryString, 'type:ajax height:600 width:996 autoFitHTML:true scrolling:no padding:0 panelPadding:20 outerClose:true controlsPos:tr ' );
	}
...
var processValidateSku = function(o)
{
    ...
    if (result == 'false') {  //if not valid, set the vld flag to 1
        ...
        //fb.start( '/catalog/eob_product.jsp'+params, 'type:ajax height:600 width:996 autoFitHTML:true scrolling:no padding:0 panelPadding:20 outerClose:true controlsPos:tr afterItemStart:updateClose() ' );
...
function displayDefaultValuesOnEdit() {
	...
	var thresholdQty = 10 || '<%=Integer.parseInt(GlobalConfig.getProperty(SiteKeys.InventoryMessage.MINIMUM_VELOCITY))%>';
	...
	// new code here to get Cut type and get Size and Color for drop downs	
	...
	if(defaultImageUrl !=""){		
		...
		var mainImageSrcLarge = defaultImageUrl.replace('$mainImage$&wid=400&hei=496','&rgn0,0,0,0');	
		jQuery('#itemZoom').attr('href',mainImageSrcLarge);
		MagicZoomPlus.update('itemZoom', mainImageSrcLarge, mainImageSrc, 'disable-expand:true; zoom-position: inner; initialize-on: hover; selectors-class : Active; click-to-activate: false');
		zoomImageCheckPP();
...
var processLoadBuyAnotherItem = function(o) {
   ...
   if (x.item.clearanceFlag)
	...
	if (x.item.itemType == "EOB" || x.item.originalItemType == "EOB") {
		...
		//else
		//	YAHOO.ebauer.layerbox.transition('eobproduct', x.item);
		fb.start( '/catalog/eob_product.jsp?standalone=N&'+queryString, 'type:ajax height:600 width:996 autoFitHTML:true scrolling:no padding:0 panelPadding:20 outerClose:true controlsPos:tr ' );
	}
	else {
		...
		fb.start( '/catalog/product.jsp?standalone=N&ensembleId='+ensembleId+'&'+queryString, 'type:ajax height:1200 width:996 autoFitHTML:true scrolling:no padding:0 panelPadding:20 outerClose:true controlsPos:tr ' );		
    }
};
