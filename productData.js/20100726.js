...
var responseFailure = function(o) {
	// alert("Error occured in : " + o.argument.processType + "<BR> More Details: " + o.statusText);
};
...
function buildZoomImage(imageURL) {
    ...
    var flashvars = {};
		flashvars['from'] = escape(window.location);
		flashvars['image'] = escape(imageURL);
		flashvars['policy'] = escape("http://s7d2.scene7.com/crossdomain.xml");
	var params = {};
		params['allowscriptaccess'] = "always";
		params['wmode'] = "transparent";
		params['scale'] = "noscale";
	swfobject.embedSWF(swfName, "prodImageContainer", "400", "500", "8", "/expressInstall.swf", flashvars, params);
}
...
function displayDefaultValuesOnEdit() {
	...
	if (itemForEditValues.item.inseamflag == 'Y') {
		...
		if ($("hemStyle").value != "Unfinished" && $("hemStyle").value != "default") {
			updateCountExpected = 3;
			getInseamLength($("hemStyle"));
		}
	}

	if (!checkIfSubscribed(YAHOO.ebauer.canvas.onCanvasInit)) {
		if (!YAHOO.ebauer.productUtils.editProductMode && itemForEditValues.item.defaultImageUrl != '' && itemForEditValues.item.defaultColorName != '') {
			YAHOO.ebauer.canvas.onCanvasInit.subscribe( function(){
				changeMainImage(itemForEditValues.item.defaultImageUrl, itemForEditValues.item.defaultColorName);
			});
		};

		YAHOO.ebauer.canvas.onCanvasInit.subscribe( function(){
			displaySelectedValuesOnEdit(updateCountExpected);
		});
	}
}
...
var processLoadBuyAnotherItem = function(o) {
   ...
   if (x.item.clearanceFlag)
	...
	if (x.item.itemType == "EOB" || x.item.originalItemType == "EOB") {
        ...
        queryString += 'categoryName=' + categoryName + '&productId=' + x.item.productid;
        //BUG00248
        queryString += '&selectedColorId=' + x.item.colorid + '&selectedSizeId=' + x.item.sizeid;
        ...
        //alert("productdata.js: processLoadBuyAnotherItem.queryString = " + queryString);
    }
};
