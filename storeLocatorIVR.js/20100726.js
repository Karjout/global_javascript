...
function OnPageLoadIVR()
	...
	radiusIVR = GetUrlVariable("radius");
	var callbackLocatorIVR =
	{
	        success : handleRawResultsIVR,
	        failure : responseFailureIVR,
	        argument: {},
	        timeout : 6000
	}
				            
	if (zipIVR !=""){
		var params = 'ebst=zc&zc='+zipIVR;			
		var reqUrl = '../../SearchService?' + params;	
		if (radiusIVR =="") {	
			radiusIVR = 50;//set default radius to 50 miles	
		}	
			reqUrl += "&r=" + radiusIVR;			
		YAHOO.util.Connect.asyncRequest('GET', reqUrl, callbackLocatorIVR);		 			 
	}else{
